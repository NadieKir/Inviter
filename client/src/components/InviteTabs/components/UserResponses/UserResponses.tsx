...
export function UserResponses() {
    ...
    const groupedEventResponses = groupBy(eventResponses, r => r.invite.event!._id);
    const inviteEventResponses: InviteEventResponse[] = Object.values(groupedEventResponses).map(gr => ({
        event: gr[0].invite.event!,
        inviters: gr.flatMap(r => r.invite),
    }));

    const responses = [...mockedInviteResponses, ...inviteEventResponses].sort((a, b) => {
        ...
        <div className={styles.userResponses}>
            {responses.map(r => (
                'inviters' in r
                    ...
                    : <InviteResponseCard key={r.invite._id} inviteResponse={r} />
            ))}
        </div>
    );
}