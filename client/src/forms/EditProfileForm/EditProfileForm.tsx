...
import { selectOptionValidationSchema, ageRangeValidationSchema } from "common/constants";
import { AgeRangeField, FormikStepper, IStep, ImageUploader, Select, TextField } from "components";
...
export function EditProfileForm({
    ...
    const secondStepInitialValues = {
        [RegistrationFormFields.City]: createOption((initialValues.city as string) ?? ''),
        [RegistrationFormFields.Orientation]: createOption((initialValues.city as string) ?? ''),
        [RegistrationFormFields.FamilyStatus]: createOption((initialValues.familyStatus as string) ?? ''),
        [RegistrationFormFields.AlcoholAttitude]: createOption((initialValues.alcoholAttitude as string) ?? ''),
        [RegistrationFormFields.SmokingAttitude]: createOption((initialValues.smokingAttitude as string) ?? ''),
        [RegistrationFormFields.Languages]: initialValues.languages.map(l => createOption(l as string)),
        [RegistrationFormFields.Interests]: initialValues.interests.map(l => createOption(l as string)),
    };
    ...
    const handleSubmit = async (values: EditProfileFormData, actions: FormikHelpers<EditProfileFormData>) => {
        ...
        var user = await updateUserProfile(resultValues);
