...
export const FollowingsPage = () => {
  ...
  const renderFollowingsAndUsers = () => {
    ...
    const userFollowingsToShow = userFollowings.filter(u => {
      ...
      return u.name.toLowerCase().includes(filterToUse) || u.login.toLowerCase().includes(filterToUse);
    });
    ...
    return (
      <>
        ...
        {usersToShow.length === 0 || <span className={styles.otherUsersLabel}>Другие пользователи:</span>}
        {usersToShow?.map(u => (
