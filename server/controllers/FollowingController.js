...
export const getFollowings = async (req, res) => {
    try {
        ...
        const followingUsers = followings.map(f => f.followingUser)
...
export const getFollowingsInvites = async (req, res) => {
    try {
        const userId = req.userId;
        const followings = await FollowingModel.find({ user: userId }, { followingUser: 1, _id: 0 }).exec()
        const followingUserIds = followings.map(f => f.followingUser);

        const followingsInvites = await InviteModel
            .find({ creator: { $in: followingUserIds } })
            .sort({ createdAt: -1 })
            .populate("creator")
            .exec();
